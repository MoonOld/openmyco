...
**要求**：快速返回，只包含基本信息、关联节点标题列表、和内部子话题标题。
...
**分类规则（严格遵守）**：
- **prerequisites**：学这个之前需要先掌握的具体知识（外部依赖）
- **postrequisites**：学完这个之后能去学的具体知识
- **related**：平行独立但有关联的具体知识
- **subTopics**：当前知识点自身的内部构成，学它就是在学这些东西

**禁止**：
- ❌ 不要把「包含当前知识的上层概念」放进任何分类（如学「集合论」时，「离散数学」是上层容器，不列）
- ❌ 不要把「当前知识的内部构成」放进关联节点（如学「React Hooks」时，「useState」是内部构成，应放 subTopics）
- ❌ 不要放过于抽象的概念（如「数学」「计算机科学」）
- ❌ 同一标题不得同时出现在关联节点和 subTopics 中

**数量**：关联节点 3-6 个，subTopics 2-5 个（如不适用可返回空数组）
...
` : ''}${subTopicTitles && subTopicTitles.length > 0 ? `
**已确定的子话题标题**（必须且只能为以下标题补充 description 和 keyPoints，不要增加或删除标题）：
${subTopicTitles.map((t) => `- ${t}`).join('\n')}
` : ''}
...
- **postrequisites**：学完这个之后能去学的具体知识
- **related**：平行独立但有关联的具体知识
- **subTopics**：当前知识点自身的内部构成

**禁止**：
- ❌ 不要把上层容器概念放进关联节点
- ❌ 不要把当前知识的内部构成放进关联节点（应放 subTopics）
- ❌ 不要放过于抽象的概念
- ❌ 同一标题不得同时出现在关联节点和 subTopics 中

**数量**：关联节点 3-5 个，subTopics 2-4 个（如不适用可返回空数组）
